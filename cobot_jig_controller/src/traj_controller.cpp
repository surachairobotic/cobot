...
#define VELOCITY_ZERO_TOLERANCE 0.025
...
//    enableMaxAngleDistSafety(_armAngleSafetyLimit > 0),
    ...
    initialized = false;
    simplifyTrajectoryVelocities = true;
...
//  trajectory_position_mode = DEFAULT_TRAJECTORY_POSITION_MODE;
  useOnlineVelocityControl = !DEFAULT_TRAJECTORY_POSITION_MODE;
	positionMode = DEFAULT_TRAJECTORY_POSITION_MODE;
//  exceed_duration_wait_factor = 1.2;
//  maxVel.resize(numTotalJoints, DEFAULT_MAX_VEL);

//  targetPos.resize(numTotalJoints, 0);
//  targetVel.resize(numTotalJoints, 0);
//  currentAngles.resize(numTotalJoints, 0);
//  currentVels.resize(numTotalJoints, 0);
}

TrajectoryController::~TrajectoryController() {}

void TrajectoryController::actionCallback()
{
	ROS_INFO("TrajectoryController::actionCallback()");
  if (hasCurrentGoal())
  {
...
//  trajectory_msgs::JointTrajectory traj = goal.getGoal()->trajectory;
  trajectory_msgs::JointTrajectory traj = control.get_trajectory();
  ...
  if (traj.points.empty())
  {
    ...
    return;
		...
		ROS_INFO("traj.points not empty");
...
void TrajectoryController::setHasCurrentGoal(bool flag)
{
  goalLock.lock();
  has_goal = flag;
  goalLock.unlock();
...
bool TrajectoryController::hasCurrentGoal()
{
  goalLock.lock();
  bool ret = has_goal;
  goalLock.unlock();
...
void TrajectoryController::adaptTrajectoryAngles(trajectory_msgs::JointTrajectory& traj, double diff, double epsilon) const
{
	ROS_INFO("TrajectoryController::adaptTrajectoryAngles");
...
bool TrajectoryController::adaptTrajectoryVelocitiesToLinear(trajectory_msgs::JointTrajectory& traj,
    const std::vector<int>& joint_indices) const
{
	ROS_INFO("TrajectoryController::adaptTrajectoryVelocitiesToLinear");
...
//		ROS_INFO("for i = %d", i);
    ...
    {
//			ROS_INFO("in - if lastPoint.velocities.size() != lastPoint.positions.size()");
      lastPoint.velocities.resize(lastPoint.positions.size(), 0);
    }
//		else
//			ROS_INFO("out - if lastPoint.velocities.size() != lastPoint.positions.size()");
    ...
    {
//			ROS_INFO("i, j(%d, %d) : %d", i, j, joint_indices[j]);
      float p1 = lastPoint.positions[j];
//			ROS_INFO("pass : p1");
      float p2 = point.positions[j];
//			ROS_INFO("pass : p2");
      float distance = MathFunctions::angleDistance(p1, p2);
//			ROS_INFO("pass : distance");
//			ROS_INFO("maxVelocities.size() : %d", maxVelocities.size());
      float maxVel = maxVelocities[joint_indices[j]];
//			ROS_INFO("pass : maxVel");
      float velocity;
      ...
      {
//				ROS_INFO("in - (fabs(v2) > ZERO_EPS) && (sign(distance)!=sign(v2))");
...
//			else
//				ROS_INFO("out - (fabs(v2) > ZERO_EPS) && (sign(distance)!=sign(v2))");

      if (fabs(timeDiff) < ZERO_EPS)
      {
//				ROS_INFO("in - fabs(timeDiff) < ZERO_EPS");
        if (fabs(distance) > ZERO_EPS)
        {
//					ROS_INFO("in - fabs(distance) > ZERO_EPS");
        ...
        {
//					ROS_INFO("out - fabs(distance) > ZERO_EPS");
          velocity = 0;
        }
      }
      else
      {
//				ROS_INFO("out - fabs(timeDiff) < ZERO_EPS");
...
bool TrajectoryController::checkTrajectory(const trajectory_msgs::JointTrajectory& traj,
    const std::vector<int>& joint_indices, int group)
{
	ROS_INFO("TrajectoryController::checkTrajectory");
  ...
  valueLock.lock();
  std::vector<double> currentAngles = current_joint_angles;
  valueLock.unlock();
  ...
  std::vector<double> firstTargetAngles;
...
bool TrajectoryController::removeIrrelevantStates(const std::vector<int>& idx, std::vector<double>& values) const
{
	ROS_INFO("TrajectoryController::removeIrrelevantStates");

  if (idx.size() != values.size()) return false;

  std::vector<double>::iterator fit = values.begin();
...
bool TrajectoryController::getTargetAngles(const trajectory_msgs::JointTrajectoryPoint& p,
    const std::vector<int>& j_idx, std::vector<double>& targetAngles) const
{
	ROS_INFO("TrajectoryController::getTargetAngles");

  if (j_idx.size() != numTotalJoints)
  {
    return false;
  }
...
bool TrajectoryController::angleDistanceOK(std::vector<double>& j1, std::vector<double>& j2,
        int check, float maxAngle) const
{
	ROS_INFO("TrajectoryController::angleDistanceOK");
  ...
  int init = numArmJoints;
  if (check == 2) init = 0;

  // Check finger angles:
  for (int i = init; i < (init + numGripperJoints); ++i)
  {
    float f1 = j1[i];
    float f2 = j2[i];
...
bool TrajectoryController::getTargetVelocities(const trajectory_msgs::JointTrajectoryPoint& p,
  const std::vector<int>& j_idx, std::vector<double>& targetVels) const
{
	ROS_INFO("TrajectoryController::getTargetVelocities");
...
bool TrajectoryController::maxVelExceeded(std::vector<double>& vels, int check, float tolerance_above) const
{
	ROS_INFO("TrajectoryController::maxVelExceeded");
...
void TrajectoryController::setTargetValues(const trajectory_msgs::JointTrajectoryPoint& p,
    const std::vector<int>& joint_indices,
    const int group, const std::vector<double>& currentState,
    std::vector<double>& targetState, bool usePositions)
{
	ROS_INFO("TrajectoryController::setTargetValues");

  if (joint_indices.size() != numTotalJoints)
  {
    ROS_ERROR_STREAM("Can't set target values because index array is not of size "<<numTotalJoints);
    return;
  }
  if (currentState.size() != numTotalJoints)
  {
    ROS_ERROR_STREAM("Can't set target values because current state is not of size "<<numTotalJoints);
    return;
  }

  if (targetState.size() != numTotalJoints)
  {
    if (usePositions)
    {
      targetState.clear();
    ...
    {
      targetState.resize(numTotalJoints, 0);
    }
  }
  ...
  {
    for (int k=numArmJoints; k<numTotalJoints; ++k) 
    {
      double v = (*val)[joint_indices[k]];
      targetState[k] = v * mult;
    }
...
bool TrajectoryController::equalJointFloats(const std::vector<double>& first, const std::vector<double>& second,
    float tolerance, bool useMinFingers)
{
	ROS_INFO("TrajectoryController::equalJointFloats");
  ...
  for (int i = 0; i < minSize; ++i)
  {
    ...
    float tol = tolerance;
    if (useMinFingers && (i >= numArmJoints)) tol = std::max(tolerance,
...
bool TrajectoryController::atTrajectoryStart(const trajectory_msgs::JointTrajectory& traj,
      const std::vector<int>& joint_indices, int group, float tolerance)
{
	ROS_INFO("TrajectoryController::atTrajectoryStart");
  ...
  valueLock.lock();
  std::vector<double> curr_angles = current_joint_angles;
  valueLock.unlock();
  goalLock.lock();
  if (traj.points.empty())
  {
...
void TrajectoryController::setExecutionFinished(bool flag, bool success)
{
	ROS_INFO("TrajectoryController::setExecutionFinished");
...
bool TrajectoryController::playTrajectoryImplementation(const trajectory_msgs::JointTrajectory& traj,
  const std::vector<int>& joint_indices, const int group)
{
	ROS_INFO("TrajectoryController::playTrajectoryImplementation");

  if (useOnlineVelocityControl)
  {
		playTrajectoryOnlineControlled(traj, joint_indices, group, INTER_GOAL_TOLERANCE, 0.0);
  }
  else
  {
		playTrajectorySimple(traj, joint_indices, group);
  }

  return true;
}

void TrajectoryController::playTrajectoryOnlineControlled(const trajectory_msgs::JointTrajectory traj,
  const std::vector<int>& joint_indices, const int group, const float inter_tolerance, float lagTime)
{
	ROS_INFO("TrajectoryController::playTrajectoryOnlineControlled");
  ...
  for (int i = 0; i < traj.points.size(); i++)
  {
    ...
    std::vector<double> _lastPos, _targetPos, _targetVel, _currAngles;
    ...
    if (i < traj.points.size() - 1)
    {
      ...
      for (int j=0; j<numTotalJoints; ++j) str<<_targetPos[j]<<" ";
  ...
  _targetVel.resize(numTotalJoints, 0);
  for (int k = 0; k < 0; ++k)
  {
    _targetVel[k] = 0;
  }
  ...
  // targetPos will still be set to the last trajectory point.
	control.move_velo(targetVel);
//	control.move_pos_velo(targetPos, targetVel);
  valueLock.unlock();
...
bool TrajectoryController::waitUntilPointReached(const std::vector<double>& _targetPos,
    const std::vector<double>& _initialTargetVel,
    float recheckTime, float maxWaitTime, float tolerance,
    float lagTime, float min_correct_vel, float max_correct_vel)
{
	ROS_INFO("TrajectoryController::waitUntilPointReached");

  std::vector<double> _targetVel = _initialTargetVel;
  ...
  while (timeSlept < maxWaitTime)
  {
    ...
    if (lagTime > ZERO_EPS)
    {
      for (int k = 0; k < numTotalJoints; ++k)
      {
    ...
    // targetPos and _targetVel are now of size numTotalJoints
    ...
    for (int k = 0; k < numTotalJoints; ++k)
    {
      ...
      float tol = tolerance;
      if (k >= numArmJoints) tol = std::max(tolerance, static_cast<float>(MIN_FINGERS_TOLERANCE));  // fingers can't do higher accuracy than this
...
//		control.move_pos_velo(targetPos, targetVel);
		control.move_velo(targetVel);
    valueLock.unlock();
		ROS_INFO("P[%lf,%lf,%lf,%lf,%lf,%lf] V[%lf,%lf,%lf,%lf,%lf,%lf]", 
						 targetPos[0], targetPos[1], targetPos[2],
						 targetPos[3], targetPos[4], targetPos[5],
						 targetVel[0], targetVel[1], targetVel[2],
						 targetVel[3], targetVel[4], targetVel[5]);
...
bool TrajectoryController::repeatedWaitUntilPointReached(const std::vector<double>& _targetPos, float recheckTime,
...
{
	ROS_INFO("TrajectoryController::repeatedWaitUntilPointReached");
  ...
  while (!equalJointFloats(_currAngles, _targetPos, tolerance, true))
  {
    ...
    std::vector<double> _targetVel;
    _targetVel.resize(numTotalJoints, 0);  // start off with 0 velocities, and leave adjustments to internal waitUntilPointReached call.
...
bool TrajectoryController::goalActive()
{
	ROS_INFO("TrajectoryController::goalActive");
...
bool TrajectoryController::waitUntilVelocitiesZero(float recheckTime, float maxWaitTime, float tolerance)
{
	ROS_INFO("TrajectoryController::waitUntilVelocitiesZero");

  bool success = true;
  float timeSlept = 0;
  int numIter = 0;
  std::vector<double> _currVels;
  float failVel = 0;
  while (timeSlept < maxWaitTime)
  {
    ++numIter;
...
void TrajectoryController::maxEndpointDiff(float& maxAngle, int& maxJoint, const trajectory_msgs::JointTrajectory& traj,
    const std::vector<int>& joint_indices, const int group)
{
	ROS_INFO("TrajectoryController::maxEndpointDiff");
...
void TrajectoryController::maxAngleDiff(const std::vector<double>& first,
    const std::vector<double>& second, float& maxVal, int& idx)
{
	ROS_INFO("TrajectoryController::maxAngleDiff");
...
void TrajectoryController::playTrajectorySimple(const trajectory_msgs::JointTrajectory traj,
    const std::vector<int>& joint_indices, const int group)
{
	ROS_INFO("TrajectoryController::playTrajectorySimple");
...
bool TrajectoryController::currentTargetReached(const trajectory_msgs::JointTrajectoryPoint& point,
      const std::vector<int>& joint_indices, const int group, float tolerance)
{
	ROS_INFO("TrajectoryController::currentTargetReached");
...
bool TrajectoryController::currentTargetReached(const std::vector<double>& target_angles, float tolerance)
{
	ROS_INFO("TrajectoryController::currentTargetReached");
...
void TrajectoryController::updateCurrentState()
{
	ROS_INFO("TrajectoryController::updateCurrentState");
  sensor_msgs::JointState joint_state;
  control.wait_new_joint_state(&joint_state, 1.0);
	current_joint_angles = joint_state.position;
	current_joint_vels = joint_state.velocity;
	ROS_INFO("P[%lf, %lf, %lf, %lf, %lf, %lf] V[%lf, %lf, %lf, %lf, %lf, %lf]", 
					 current_joint_angles[0], current_joint_angles[1], current_joint_angles[2],
					 current_joint_angles[3], current_joint_angles[4], current_joint_angles[5],
					 current_joint_vels[0], current_joint_vels[1], current_joint_vels[2],
					 current_joint_vels[3], current_joint_vels[4], current_joint_vels[5]);
}
