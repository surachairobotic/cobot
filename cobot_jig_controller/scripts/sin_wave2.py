...
def find_pid(pub_target, pub, kp, kpp, ki, fp):
  global save_file
  rate = rospy.Rate(100) # 100hz
...
#  amp = 1.0
  ...
  jntHome.position[numJnt] = math.radians(90)-((math.radians(180)-(2*amp))/2.0)
  if nameJnt is 'J5':
...
#  ki = 0.005
  ...
  set_jnt(jntHome, pub)
  t_start = time.time()
  while (time.time()-t_start) < 2.5 and not rospy.is_shutdown(): a=0
...
#  jntHome.position[1] += math.radians(90)
  set_jnt(jntHome, pub)
  t_start = time.time()
  while (time.time()-t_start) < 4.0 and not rospy.is_shutdown(): a=0
...
###
  ...
  jntGo.velocity = [0.0]
  if save_file:
    fp.write("%.1f,%.1f,%.3f,%f,%f\r\n" % (kp, kpp, ki, sum(rms_v)/len(rms_v), sum(rms_p)/len(rms_p)))
  print("kp=%.1f,kpp=%.1f,ki=%.3f|rms|v=%f,p=%f" % (kp, kpp, ki, sum(rms_v)/len(rms_v), sum(rms_p)/len(rms_p)))
  set_jnt(jntGo, pub)
...
if __name__ == '__main__':
  ...
  Kp = [0.1,0.3,0.5,0.7]
  Kpp = [0.1,0.3,0.5,0.7,1.0]
  Ki = [0.005,0.01,0.05]

#  for kp in Kp:
#    for kpp in Kpp:
#      for ki in Ki:
#        find_pid(pub_target, pub, kp, kpp, ki, fp)
  find_pid(pub_target, pub, 0.0, 0.0, 0.0, fp)
