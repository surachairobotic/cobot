...
def check_tq():
  ...
  if first:
    ...
    dt = (state.header.stamp-old_state.header.stamp).to_sec()
  ...
  #      TqCurrent = current2torque(arm_state.effort[4], arm_state.velocity[4])
    vars = eq.get_vars(state.position) # load equation parameters for calculating torque
    ...
    err = [[], [], [], [], [], []]
    for i in range(6):
      list_current_cal[i].append(torque2current(torque_with_ddq[i], state.velocity[i], ABC[i]))
  ...
  #    stat.position = [float(current_cal), float(eff[numJnt]), float(y1[indx-1]), float(y2[indx-1]), float(y3[indx-1])]
    ...
    stat.header.stamp = state.header.stamp
    stat.effort = eff
  ...
  old_vel = np.array(state.velocity)
  old_state = state
  first = True
